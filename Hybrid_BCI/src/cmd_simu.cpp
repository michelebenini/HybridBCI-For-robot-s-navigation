...
int main(int argc, char **argv)
{
  ...
  player pls[config.max_p300];
  for(int i = 0; i < config.max_p300; i++){_init_player(&pls[i],i);}
  ...
  while (ros::ok())
  {
    ...
    if( c == 'd'){
      ...
      msg.tot_people = config.max_p300;
      for(int i = 0; i < config.max_p300; i++){
...
void p300_set_fixed(struct player *pls, int n){                          // set p300 probability in a prefixed way
  ...
  for(int i = 0; i < config.max_p300; i++){
    if( i == n ){
      pls[i].p = config.fixed_p300;
    }
    else{
      pls[i].p = (1-config.fixed_p300)/(config.max_p300-1);
